	...
	*/
	function getUserContacts()
	{
		...
		foreach( $response["data"] as $item ){
			...
			$uc->photoURL    = "https://api.facebook.com/" . $uc->identifier . "/picture?width=150&height=150";
	...
	function getUserActivity( $stream )
	{
		...
		foreach( $response['data'] as $item ){
			...
			if( ! empty( $ua->text ) ){
				...
				$ua->user->photoURL     = "https://api.facebook.com/" . $ua->user->identifier . "/picture?type=square";
